...
func TestUpdateAllWithInlinePGP(t *testing.T) {
	...
	changed = UpdateAll(notesDir, cacheFile, keyIDs)
	if len(changed) != 0 {
		t.Errorf("expected 0 changes on second run, got %d", len(changed))
	}
}
...
func TestUpdateManyFilesWithInlinePGP(t *testing.T) {
	...
	if len(changed) != 3 {
		t.Errorf("expected 3 changes from UpdateManyFiles, got %d", len(changed))
	}
...
func TestCanDecryptAndMissingKey(t *testing.T) {
	gnupgHome, keyIDs := setupTestGPG(t)
