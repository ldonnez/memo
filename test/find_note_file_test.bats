...
@test "retuns the path if giving relative path of notes dir" {
  ...
  run _find_note_file "test_dir/test.md"
  assert_success
  assert_output "$file"
}

@test "retuns the given file name in subdir when working dir is subdir" {
  mkdir -p "$NOTES_DIR/test_dir"
